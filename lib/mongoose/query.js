 ...
 * @param {Object} obj
 * @return {Object}
 * @TODO test $pull, $pullAll, all $atomics
 * @TODO test default vaues are set and saved
 * @TODO document well
 */
...
Query.prototype._castUpdate = function (obj) {
  ...
  while (i--) {
    ...
    // if its an $atomic method, apply using model instance methods
    if ('$' === path[0]) {
      // scenario: val must be an object, keys of val must be paths or throw
      ...
      while (ii--) {
        ...
        //console.error(subpath, method);
        var pathexists = schema.path(subpath);
        //console.error('%s path exists? %s', subpath, !!pathexists);

        if (pathexists) {
          if ('$set' === method) {
            //console.error('doc.set(%s %j)', subpath, subval);
            doc.set(subpath, val);
            continue;
          }
          ...
          //console.error('checking for method %s', method);
          ...
          if ('function' === typeof cur[method]) {
            //console.error('found $atomic method: doc.%s.%s(%j)', subpath, method, subval);
            cur[method](subval);
          }
        }
      }
